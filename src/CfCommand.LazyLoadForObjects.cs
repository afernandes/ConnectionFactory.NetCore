                        ...
                        for (var index = 0; index < reader.FieldCount; index++)
                        {
                            if (properties.ContainsKey(reader.GetName(index).ToUpper()))
                            {
                                ...
                                if ((info != null) && info.CanWrite && !reader.IsDBNull(index))
                                {
                                    ...
                                    {
                                        //Logger.LogError(ex, string.Format("LazyLoadForObjects<{0}>", typeof(T).Name), cmdType, cmdText, cmdParms);
                            ...
                            {
                                //Logger.LogDebug("Property not exist: " + entityType.FullName + reader.GetName(index),null);
                            }
                        }
